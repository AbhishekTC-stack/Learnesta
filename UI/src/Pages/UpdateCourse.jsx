...
const UpdateCourse = () => {
  ...
  // When a course is selected fill the form with its existing data
  const handleCourseSelect = (e) => {
    const id = e.target.value;
    setSelectedCourseId(id);
    const found = courses.find((c) => c._id === id);
    if (found) {
      setCourseName(found.CourseTitle || "");
      setCourseId(found.CourseId || "");
      setCourseType(found.CourseType || "Self-Paced");
      setDescription(found.Description || "");
      setPrice(found.Price || 5000);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/admin/updatecourse/${selectedCourseId}`, {
        method: "PATCH",
        credentials: "include",
      ...
      if (!res.ok) {
        ...
        throw new Error(err.msg || "Error updating course");
      }
    ...
    <div className="flex h-screen bg-gray-200">
      ...
      <div className="flex-1 flex flex-col items-center justify-start pt-16">
        ...
        <div className="flex rounded-full shadow-md overflow-hidden mb-16">
          ...
          <button
            className="px-10 py-3 bg-gradient-to-r from-purple-700 to-purple-900 text-white font-semibold"
          >
            UPDATE COURSE
          </button>
        </div>
        ...
        <div className="bg-gray-100 rounded-3xl shadow-2xl w-[600px] p-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <label className="w-40 text-sm font-semibold">SELECT COURSE</label>
              <select
